...
def makeProductCollection(colOptions,
    ...
    if (input_collection.size().getInfo() > 0):
        ...
        if variableName == "Surface_Reflectance":
            print(variableName)
            outputs = input_collection.map( lambda image: image.setDefaultProjection(crs=image.select(image.bandNames().slice(3,4)).projection()) \
            ...
            estimateSL2P = input_collection.map(lambda image: toolsNets.wrapperNNets(SL2P,partition, netOptions, colOptions,"estimate",variableName,image))
            uncertaintySL2P = input_collection.map(lambda image: toolsNets.wrapperNNets(SL2Puncertainty,partition, netOptions, colOptions,"error",variableName,image))
            outputs =  outputs.combine(estimateSL2P).combine(uncertaintySL2P.select("error"+variableName)) \
...
def sampleProductCollection(productCollection, sampleRegion, outputScale, factor=1,numPixels=0,geometries=False) :
    ...
    if (numPixels>0):
        sampleData = productCollection.map(lambda image: image.sample(region=sampleRegion.geometry(),\
                            projection=image.select(image.slice(4,5).bandNames()).projection(), scale=outputScale,geometries=geometries, dropNulls = True, numPixels=numPixels) ).flatten()
    else:
        sampleData = productCollection.map(lambda image: image.sample(region=sampleRegion.geometry(),\
                            projection=image.select(image.slice(4,5).bandNames()).projection(), scale=outputScale,geometries=geometries, dropNulls = True, factor=factor) ).flatten()
    return sampleData
...
def getSamples(site,
    ...
    if productCollection :
        if ( productCollection.size().getInfo() > 0 ):
            outputFeatureCollection = ee.FeatureCollection(sampleProductCollection(productCollection, site.geometry(),  outputScale,factor,numPixels,geometries))
...
def sampleSites(siteList,
    ...
    for input in siteList:   
        ...
        for n in range(featureRange[0],featureRange[1]) :
            ...
            if (len(pd.date_range(startDate,endDate,freq=shardSize)) > 0 ) & (defaultDate==True) & (shardSize != None) :
            ...
            for index, Dates in dateRange.iterrows():
                ...
                if outputFeatureCollection :
                    ...
                    if ( exportFlag in ['Asset','Drive','CloudStorage'] ) :
            ...
            for index, Dates in dateRange.iterrows():
                ...
                if outputImageCollection :
                    ...
                    if  ( exportFlag in ['Asset'] ):
                        ...
                        tasks = ee.batch.Export.geetools.imagecollection.toAsset(**export_params)
